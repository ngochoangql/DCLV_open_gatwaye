...
class BaseReader(object):
    ...
    def __init__(self, config, device_id=None, name=None, **kwargs):
        ...
        self.data_type = config.get("DATA_TYPE", "float")
    ...
    def read_config(self):
        ...
        print(self.config_columns)
        self.data_type = config.get("data_type", "float")
        print("config",self.config_columns)
        return config
    ...
    def convert_data_to_list(self, data):
        ...
        print(len(data),num_samples,self.data_byte_size)
        print(self.data_type_str)
        tmp = struct.unpack(self.data_type_str * num_samples, data)
        print("TMP",tmp,self.scaling_factor)
        tmp = [x for x in tmp]
        print("TMP1",tmp)
    ...
    def execute_run_sml_model(self, sml, data):
        for data_chunk in self.convert_data_to_list(data):
            ...
            if ret >= 0:
                ...
                sml.reset_model(0)
    def execute_run_sml_model_recognition(self, sml, data):
        for data_chunk in self.convert_data_to_list(data):
            ...
            print("********************",ret,data_chunk)
            if ret >= 0:
                a = self._map_classification({"ModelNumber": 0, "Classification": ret})
